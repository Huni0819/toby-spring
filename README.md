...
### Spring Apllication의 계층 구조

1. Presentation Layer
   - @Controller
2. Business(Service, Application) Layer
   - @Service
   - 전체 애플리케이션의 핵심적인 비즈니스 로직이 들어감
3. Persistence(Data Access, Infra) Layer
   - @Repository

> 서비스 추상화에서 말하는 것은 @Service 어노테이션이 붙은 걸 말하지 않는다.
> 서비스라는 것은 일반적인 용어라서 쓰이는 곳에 따라 의미가 달라진다.

- 서비스는 클라이언트에게 서비스를 제공해주는 오브젝트나 모듈
  - 서버라고 불러도 됨
  - 모든 서비스에는 클라이언트가 존재해야 한다.
- 서비스는 상태를 가지지 않는다.
  - 오브젝트가 속성값을 저장해두지 않는다.
  - stateless 싱글톤 스프링 빈을 사용하기에 적합하다.

- 서비스의 종류
  1. 애플리세이션 서비스
     - @Service 어노테이션이 붙는다.
  2. 도메인 서비스
     - 도메인 모델 패턴을 이용해 비즈니스 로직을 엔티티 같은 도메인 오브젝트에 넣는 경우에
     - 특정 엔티티로는 표현하기 힘든 로직이 있다.
     - 이 경우 도메인 서비스의 오브젝트를 만들어 여기에 넣을 수 있다.
  3. **인프라 서비스 infrastructure service**
     - 기술을 제공하는 서비스
     - 메일, 캐시, 트랜잭션, 메시징 등등 서비스 추상화 대상

### 애플리케이션 서비스 도입

1. 애플리케이션 혹은 비즈니스 로직이 들어간다.
2. 특정 기술의 변경이나 특정 기술이 적용되어진 환경에 의존하지 않도록 만들어야 한다.
...
1. Order class
   ...
   - 그럼에도 제거하고 싶다면 외부 XML 디스크립터를 사용할 것.

### 트랜잭션 서비스 추상화

1. 추상화
   - 구현의 복잡함과 디테일을 숨기고 중요한 것만 남기는 기법
   - 인프라 서비스 기술의 핵심 기능을 인터페이스로 정의하고 구현하는 어댑터를 만들어 일관된 사용이 가능하게 만드는 것이 **서비스 추상화**
2. 어댑터

### JDBC 데이터 액세스 기술

1. JdbcClient
   - 일종의 Template-Callback
   - Spring 6.1에 추가되어 Spring의 JdbcTemplate을 대체할 기술이다.
2. DataSourceTransactionManager

- 구성 정보는 구현이 달라지면 바뀔 수 있다.

### 트랜잭션 테스트

1. 트랜잭션 테스트
   - 트랜잭션이 정확하게 적용되었는지 테스트하긴 어렵다.
   - 모든 작업이 성공한다면 하나의 트랜잭션으로 진행된 것인지, 여러 트랜잭션으로 쪼개진 것인지 알기가 어려움.
   - 트랜잭션 중간에 실패 케이스를 만들 수 있다면 롤백 여부를 통해서 확인이 가능하다.

### 트랜잭션 프록시

1. OrderService의 기능을 수정하지 않은채로 부가적인 기능을 더해보자.
   - 여기서 부가적인 기능은 트랜잭션.
- 데코레이터 패턴
  - 오브젝트의 코드를 수정하지 않고 새로운 기능을 런타임에 부여하는 패턴
  - 일종의 Composite 패턴과 비슷하다.
- 프록시 패턴
  - 접근 제어의 역할
  - 실제 애플리케이션 서비스 로직에 기술을 영향을 받는 기술이 변경될 때마다 바뀌어야 하는, 기술이 노출이되는 코드를 제거하고 본인의 책임과 기능에 충실한 코드만 남길수 있다. 
  - 비용이 비싼 Target에 대해서 Proxy 객체를 주입하고 실제 사용할 때 생성하게 사용할 수 있다.
  - Client에게 target인척
- 기능을 부여하는건가? 접근 제어를 하는건가? 관점에서 봐야하지만
- 뭉뚱거려서 Proxy라고 보긴함;

- 이제 OrderService는 모든 데이터 기술에 의존적인 코드를 다 제거했다.